...
export function Sidebar({ storageKey = "t-sidebar-state" }: SidebarProps) {
    ...
    const defaultAccordionValue: string[] = Object
        .keys(expanded)
    ...
    if (!organizationHook.isLoaded || !organizationList.isLoaded || userMemberships.isLoading) {
        return (
            <div>
    ...
    return <div>
        ...
        >
            {userMemberships.data?.map(({ organization }) => (
                <NavItem
                    key={organization.id}
                    isActive={organizationHook.organization?.id === organization.id}
